 ...
 * @author falk
 */
public class DisequalityGuard<T> implements SDTGuard<T> {

    private final SuffixValue<T> parameter;
    private final GuardElement register;

    public DisequalityGuard(SuffixValue<T> parameter, GuardElement register) {
    ...
    public boolean equals(Object o) {
        ...
        DisequalityGuard<?> that = (DisequalityGuard<?>) o;
    ...
    public Expression<Boolean> toExpr() {
        return new NumericBooleanExpression(parameter, NumericComparator.NE, register.asExpression());
    }

    @Override
    public SDTGuard<T> relabel(SDTRelabeling relabeling) {
        return new DisequalityGuard<>(relabeling.getIfAvailable(parameter), relabeling.getIfAvailable(register));
    }
