...
public class MultiTheorySDTLogicOracle implements SDTLogicOracle {
    ...
    @Override
    public boolean hasCounterexample(Word<? extends SymbolInstance<?>> prefix, SDT sdt1,
                                     SDT sdt2, Expression<Boolean> guard, Word<? extends SymbolInstance<?>> rep) {
        ...
        for (Variable<?> sv : ExpressionUtil.freeVariables(exprG)) {
            if (sv instanceof SymbolicDataValue.Parameter<?>) {
                SymbolicDataValue.Parameter<?> p = (SymbolicDataValue.Parameter<?>) sv;
...
//        System.out.println("HAS CE: " + test);
        boolean r = solver.isSatisfiable(test) == Result.SAT;
        LOGGER.trace("Res:" + r);
        return r;
    }

    @Override
    public Expression<Boolean> getCEGuard(Word<? extends SymbolInstance<?>> prefix, SDT sdt1, SDT sdt2) {
    	...
    	for (Map.Entry<Expression<Boolean>, Boolean> e1 : exprMap1.entrySet()) {
            ...
            for (Map.Entry<Expression<Boolean>, Boolean> e2 : exprMap2.entrySet()) {
                ...
                if (outcome1 != outcome2) {
                    ...
                    if (Result.SAT == solver.isSatisfiable(test)) {
                        return expr1;
                    }
                }
            }
    ...
    public boolean doesRefine(Expression<Boolean> refining, Expression<Boolean> refined, Mapping<SymbolicDataValue<?>, DataValue<?>> valuation) {
        ...
        boolean r = solver.isSatisfiable(SMTUtil.toExpression(test, valWithConsts)) == Result.SAT;
        return !r;
    }
    ...
    public boolean areMutuallyExclusive(Expression<Boolean> guard1, Expression<Boolean> guard2,
        ...
        boolean r = solver.isSatisfiable(SMTUtil.toExpression(test, valuation)) == Result.SAT;
        return !r;
    }

    @Override
    public boolean areEquivalent(Expression<Boolean> guard1, Bijection<DataValue<?>> remapping, Expression<Boolean> guard2,
                                 Valuation<SymbolicDataValue<?>, DataValue<?>> valuation) {
        LOGGER.trace("guard1: {0}", guard1);
        LOGGER.trace("guard2: {0}", guard2);
        ...
        Expression<Boolean> test = ExpressionUtil.or(
                ExpressionUtil.and(guard1, new Negation(g2relabel)),
                ExpressionUtil.and(new Negation(guard1), g2relabel)
        );
    ...
    @Override
    public boolean accepts(Word<? extends SymbolInstance<?>> word, Word<? extends SymbolInstance<?>> prefix, SDT sdt) {
    ...
    private boolean accepts(Word<? extends SymbolInstance<?>> word, Word<? extends SymbolInstance<?>> prefix, int symIndex, SDT sdt,
            Mapping<SymbolicDataValue<?>, DataValue<?>> valuation) {
        boolean accepts;
        if (symIndex == word.length()) {
            accepts = sdt.isAccepting();
        } else {
            SymbolInstance<?> sym = word.getSymbol(symIndex);
            if (sym.getBaseSymbol().getArity() == 0) {
                ...
                for (int i = 0; i < sym.getBaseSymbol().getArity(); i++) {
                    ...
                    for (Map.Entry<SDTGuard, SDT> entry : nextSdt.getChildren().entrySet()) {
                        ...
                        if (solver.isSatisfiable(SMTUtil.toExpression(guardExpr, newValuation)) == Result.SAT) {
